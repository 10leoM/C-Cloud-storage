...
Server::Server(EventLoop *loop, const char *ip, uint16_t port): mainReactor(loop), next_conn_id(1)
{
    ...
    std::function<void(int)> cb = std::bind(&Server::NewConnection, this, std::placeholders::_1); // 绑定新连接回调函数
...
void Server::NewConnection(int fd)
{
    ...
    connections[fd] = std::make_shared<Connection>(sub_loop, fd, next_conn_id++); // 创建新的连接对象
